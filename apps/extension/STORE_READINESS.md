...
| API default | Production webpack build: `https://api.jobloom.tech` (override: env `EXTENSION_API_BASE` at build time). **Store builds ignore** `chrome.storage.local.apiBase` and clear it on install/update so dev URLs cannot break production. Dev builds may still override via storage. |
...
| **host_permissions: `https://*/*`** | The service worker fetches your **JobLoom backend over HTTPS** (any host you configure as `https://...`). This pattern is **not** the same as `<all_urls>` (e.g. excludes `file://`), but is still a **broad** pattern. Reviewers expect a clear, accurate justification. | **Narrower:** ship only `https://api.jobloom.tech/*` (or your final API host) and **remove** `apiBase` overrides from storage in production. That reduces scope at the cost of flexibility for self-hosted/staging. |
