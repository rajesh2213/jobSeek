...
export function renderGrowthEmailHtml(input: {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>${esc(input.campaignLabel)}</title></head><body style="font-family:system-ui,Arial,sans-serif;background:#f7f8fa;padding:20px;"><div style="max-width:640px;margin:0 auto;background:#fff;border-radius:12px;padding:24px;"><table role="presentation" cellspacing="0" cellpadding="0" style="margin:0 0 10px 0;"><tr><td style="vertical-align:middle;padding-right:8px;"><img src="${esc(input.brandLogoUrl)}" alt="JobLoom logo" width="22" height="22" style="display:block;border:0;outline:none;text-decoration:none;border-radius:6px;"/></td><td style="vertical-align:middle;"><p style="margin:0;font-size:12px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#111827;">JobLoom</p></td></tr></table><p style="margin:0 0 8px 0;color:#111827;">Hi ${esc(input.userName)},</p><p style="margin:0 0 16px 0;color:#374151;">${esc(input.campaignLabel)}</p>${sections}<p style="margin-top:20px;"><a href="${esc(input.ctaUrl)}" style="display:inline-block;background:#111827;color:#fff;padding:10px 14px;border-radius:8px;text-decoration:none;">View more jobs</a></p><p style="margin:12px 0 0 0;color:#374151;">Get more personalized alerts with Pro. <a href="${esc(input.upgradeToProUrl)}" style="color:#0b57d0;text-decoration:none;">Upgrade to Pro</a>.</p><hr style="border:none;border-top:1px solid #e5e7eb;margin:22px 0;"/><p style="font-size:12px;color:#6b7280;margin:0;">Manage your emails: <a href="${esc(input.managePreferencesUrl)}">preferences</a> · <a href="${esc(input.unsubscribeUrl)}">unsubscribe</a></p></div></body></html>`;
}
