"use client";

import Script from "next/script";
import { usePathname } from "next/navigation";
import { useEffect, useRef, type ReactNode } from "react";
import { captureAndPersistAttribution } from "./attribution";
import {
  getPublicMetaPixelId,
  isClientAnalyticsAllowed,
  isMetaPixelEnabled,
} from "./config";
import { trackMetaPageView } from "./meta";

function sanitizePixelId(raw: string | null): string | null {
  if (!raw) return null;
  if (/^\d{5,24}$/.test(raw)) return raw;
  return null;
}

function MetaRoutePageViews() {
  const pathname = usePathname();
  const last = useRef<string | null>(null);

  useEffect(() => {
    if (!isMetaPixelEnabled() || !isClientAnalyticsAllowed()) return;
    if (last.current === pathname) return;
    last.current = pathname;
    trackMetaPageView();
  }, [pathname]);

  return null;
}

function MetaPixelBootstrap() {
  const pixelId = sanitizePixelId(getPublicMetaPixelId());
  if (!isMetaPixelEnabled() || !pixelId) return null;

  return (
    <Script
      id="meta-pixel-bootstrap"
      strategy="afterInteractive"
      dangerouslySetInnerHTML={{
        __html: `
!function(f,b,e,v,n,t,s)
{if(f.fbq)return;n=f.fbq=function(){n.callMethod?
n.callMethod.apply(n,arguments):n.queue.push(arguments)};
if(!f._fbq)f._fbq=n;n.push=n;n.loaded=!0;n.version='2.0';
n.queue=[];t=b.createElement(e);t.async=!0;
t.src=v;s=b.getElementsByTagName(e)[0];
s.parentNode.insertBefore(t,s)}(window, document,'script',
'https://connect.facebook.net/en_US/fbevents.js');
fbq('init', '${pixelId}');
`,
      }}
    />
  );
}

export function MetaAnalyticsProvider({ children }: { children: ReactNode }) {
  useEffect(() => {
    captureAndPersistAttribution();
  }, []);

  return (
    <>
      <MetaPixelBootstrap />
      <MetaRoutePageViews />
      {children}
    </>
  );
}
