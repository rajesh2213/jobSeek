...
/** Horizontal reference span for squiggle control points (scaled to dot → account). */
...
const CHECKPOINTS: readonly CheckpointDef[] = [
  {
    ...
    demoSubtitle: "See roles before most ATS & LinkedIn.",
  },
  {
    id: 1,
...
function buildSquigglyPathD(sx: number, sy: number, ex: number, ey: number): string {
  const W = ex - sx;
...
function dispatchWorkflowGlow(): void {
  if (typeof window === "undefined") return;
  window.dispatchEvent(
    new CustomEvent(WORKFLOW_GLOW_EVENT, {
      detail: { ms: WORKFLOW_GLOW_MS },
    }),
  );
}
...
export function HeaderWorkflow() {
  ...
  const measure = useCallback(() => {
    ...
    const endpoint = document.getElementById(WORKFLOW_ACCOUNT_ENDPOINT_ID);
    if (!dotEl || !endpoint) return;

    const dot = dotEl.getBoundingClientRect();
    const btn = endpoint.getBoundingClientRect();
  ...
  useEffect(() => {
    ...
    const run = async () => {
      while (!cancelled) {
        ...
        if (cancelled) return;
        dispatchWorkflowGlow();
        await sleep(WORKFLOW_GLOW_MS);
        if (cancelled) return;
  ...
  }, [layout, pathLen, progress, reduceMotion]);
  ...
  const dashOffsetAnimated = useTransform(progress, (p) => pathLen * (1 - p));
