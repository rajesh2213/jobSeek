...
import { WORKFLOW_GLOW_EVENT, WORKFLOW_GLOW_MS } from "../../lib/headerWorkflowGlow";

/**
 * Radial glow + ring triggered by `WORKFLOW_GLOW_EVENT` (header workflow path completion).
 * Pass `children` as a function to style the target when `glow` is true (e.g. account avatar).
 */
export function WorkflowEndpointGlow({
  ...
  useEffect(() => {
    ...
    const onGlow = (e: Event) => {
      ...
      const ms = typeof ce.detail?.ms === "number" ? ce.detail.ms : WORKFLOW_GLOW_MS;
      setGlow(true);
      if (offTimer) clearTimeout(offTimer);
      offTimer = setTimeout(() => setGlow(false), ms);
    };
    window.addEventListener(WORKFLOW_GLOW_EVENT, onGlow);
    return () => {
      window.removeEventListener(WORKFLOW_GLOW_EVENT, onGlow);
      if (offTimer) clearTimeout(offTimer);
    };
  }, []);
